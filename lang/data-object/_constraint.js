...
class Data_Object_Def_Constraint extends Constraint {
	...
	'match'(value) {
		...
		if (tv == 'object') {
			...
			each(this.data_def, function(field_def, field_name, stop) {
	...
	if (a.l == 1 && tof(a[0]) == 'array') {
		...
		if (obj_sig == '[s,n]') {
			...
			if (data_type_name == 'text') {
				constraint = new Text_Constraint({'length': length});
				return constraint;
			}
		...
		if (obj_sig == '[s,s]') {
			...
			if (constraint_type == 'unique') {
				...
				constraint = new Unique_Constraint({
					'fields': field_name
				});
...
var constraint_from_str = function(str) {
	...
	if (match_slt) {
		...
		constraint = new Text_Constraint({
			'length': length
		});
		return constraint;
	}
	...
	if (str === 'int') {
		constraint = new Int_Constraint({});
		return constraint;
	}
	if (str === 'number') {
		constraint = new Number_Constraint({});
		return constraint;
	}
	if (str === 'guid') {
		constraint = new Guid_Constraint({});
		return constraint;
	}
