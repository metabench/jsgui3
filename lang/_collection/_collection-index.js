...
// 18/12/2016 - Decided that this collection index class makes things too complicated.
//  Will make indexing and retrieval more functional. Will use some appropriate data structures within Collection, and implement indexing there.
//   Increases code locality, may well be easier to read and understand.
//   Will call upon some general purpose functions where necessary.
//    The index will just be a data structure, and it will check that data structure against keys to find the object.
...
var Data_Object = require('../data-object/data-object');
...
class Collection_Index {
	'constructor'(spec) {
		...
		if (is_defined(spec.fields)) {
			...
			// defining the fields to index by.
...
var get_obj_fields_key = (obj, fields) => {


	// Not very clear what this does or why it is necessary right now.
	...
	// var first = true;
	var arr_res = [];
	each(fields, function(field_definition, field_name) {
		...
		if (tFieldDef == 'array') {
			...
			if (obj.get) {
				...
				var field_val = obj[field_definition];
			}
			...
			if (field_definition.attached) {
				...
				var c = 0;
				each(field_definition.attached, function(v, i) {
					attachedObjName = i;
...
class Sorted_Collection_Index extends Collection_Index {
	...
	'unsafe_add_object'(obj) {
		...
		// No, does not work according to the fields?


		// May not have the fields either?
		// Possibly need to get fields working again so that the indexes will work.




		// The fields key should refer to the field name we are indexing it under.
		//  Some other code should be done differently and more simply.
		//
...
class Dict_Collection_Index extends Collection_Index {
	...
	'unsafe_add_object'(obj) {
		...
		// Need to know how the object's indexed fields are to be represented.
		//  A bit of a complicated requirement to specify and name
		//  At this stage, we need to know hoe the item gets indexed within the collection.

		// Which are the fields or values to pay attention to in the object.
		//  We know what the object gets indexed by.
		//  There could even me multiple indexes.
...
class Collection_Index_System {
	...
	'search_for_index_with_fields'(a, sig) {
		...
		if (sig == '[a]') {
			...
			console.log('idx.sorted_kvs.tree.keys()', idx.sorted_kvs.tree.keys());
			return idx;
	...
	'find'(a, sig) {
		...
		if (sig == '[o]') {
			...
			each(objQuery, function(value, key) {
				//console.log('key ' + key);
